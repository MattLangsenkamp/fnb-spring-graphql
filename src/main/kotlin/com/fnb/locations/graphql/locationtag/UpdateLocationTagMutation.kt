...
@Component
class UpdateLocationTagMutation
@Autowired constructor(private val locationTagService: LocationTagService) : Mutation {

    private val logger = LoggerFactory.getLogger(javaClass)

    suspend fun updateTag(graphQLContext: MyGraphQLContext, id: Int, tagName: String?, description: String?): LocationTag {

        val loggedInUser = graphQLContext.loggedInUser
                ?: throw NotLoggedInExceptionException("Log in to add new location")

        return locationTagService.updateTag(loggedInUser, id, tagName, description)
    }
